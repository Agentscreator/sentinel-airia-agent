...
def _get_port_file(profile: str, storage_path: Path | None) -> Path | None:
    ...
    if storage_path is None:
        storage_path_str = os.environ.get("NEXA_STORAGE_PATH", os.environ.get("HIVE_STORAGE_PATH"))
        if storage_path_str:
...
def allocate_port(profile: str, storage_path: Path | None = None) -> int:
    ...
    Args:
        ...
        storage_path: Base storage path (uses NEXA_STORAGE_PATH env if not provided)

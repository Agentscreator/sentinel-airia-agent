...
def ensure_credential_key_env() -> None:
    ...
    Priority chain for each credential:
      ...
      2. Dedicated file storage (``~/.nova-nexa/secrets/`` or encrypted store)
      3. Shell config fallback (``~/.zshrc`` / ``~/.bashrc``) for backward compat

    Boot order matters: NEXA_CREDENTIAL_KEY must load BEFORE ADEN_API_KEY
    ...
    # Step 1: NEXA_CREDENTIAL_KEY (must come first — encrypted store depends on it)
    load_credential_key()
    ...
    try:
        ...
        for spec in CREDENTIAL_SPECS.values():
            ...
            if var_name and var_name not in ("NEXA_CREDENTIAL_KEY", "ADEN_API_KEY"):
...
def validate_agent_credentials(
    ...
    if os.environ.get("NEXA_CREDENTIAL_KEY"):

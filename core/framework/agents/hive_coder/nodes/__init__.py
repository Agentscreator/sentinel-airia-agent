...
# Tools available to both coder (worker) and orchestrator.
_SHARED_TOOLS = [
    # File I/O
    "read_file",
    "write_file",
...
# Orchestrator (queen) phase-specific tool sets.
...
# orchestrator compose their system prompts from this block + role-specific
# additions.
# ---------------------------------------------------------------------------
...
**Communication style**: Be concise. Say less. Mean more. Impatient stakeholders \
...
but also always allow the user to type something beyond the options.

If a question doesn't do one of these, don't ask it. Make an assumption, state it, and move on.

---

### 1.1: Let Them Talk, But Listen Like a Solution Architect
...
**Typical patterns (orchestrator manages all user interaction):**
...
The orchestrator owns intake: it gathers user requirements, then calls \
...
match what the orchestrator will provide at run time. Worker nodes should \
...
**Please make sure you have proposed the design to the user before implementing**
...
`mcp_servers.json` is auto-generated with nexa-tools as the default. \
...
## When user asks identity and responsibility
...
# Single node — one continuous context handles the entire workflow:
...
queen_node = NodeSpec(
    ...
    skip_judge=True,  # Orchestrator is conversational; suppress tool-use pressure feedback

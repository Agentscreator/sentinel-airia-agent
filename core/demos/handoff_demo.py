...
# Add core, tools, and project root to path
_CORE_DIR = Path(__file__).resolve().parent.parent
_PROJECT_DIR = _CORE_DIR.parent
sys.path.insert(0, str(_CORE_DIR))  # framework.*
sys.path.insert(0, str(_PROJECT_DIR / "tools" / "src"))  # aden_tools.*
sys.path.insert(0, str(_PROJECT_DIR))  # core.framework.* (for aden_tools imports)
...
STORE_DIR = Path(tempfile.mkdtemp(prefix="nexa_handoff_"))
...
async def _run_pipeline(websocket, topic: str):
    ...
    run_dir = Path(tempfile.mkdtemp(prefix="nexa_run_", dir=STORE_DIR))

...
# Hive config paths
$NexaConfigDir  = Join-Path $env:USERPROFILE ".nova-nexa"
$NexaConfigFile = Join-Path $NexaConfigDir "configuration.json"
...
$PrevEnvVar = ""
$PrevSubMode = ""
if (Test-Path $NexaConfigFile) {
    try {
        $prevConfig = Get-Content -Path $NexaConfigFile -Raw | ConvertFrom-Json
...
if ($SelectedProviderId) {
    ...
    if (-not (Test-Path $NexaConfigDir)) {
        New-Item -ItemType Directory -Path $NexaConfigDir -Force | Out-Null
    }
    ...
    $config | ConvertTo-Json -Depth 4 | Set-Content -Path $NexaConfigFile -Encoding UTF8
    Write-Ok "done"
...
if (Test-Path $NexaConfigFile) {
    $existingConfig = Get-Content -Path $NexaConfigFile -Raw | ConvertFrom-Json
    $existingConfig | Add-Member -NotePropertyName "gcu_enabled" -NotePropertyValue $true -Force
    $existingConfig | ConvertTo-Json -Depth 4 | Set-Content -Path $NexaConfigFile -Encoding UTF8
} else {
    if (-not (Test-Path $NexaConfigDir)) {
        New-Item -ItemType Directory -Path $NexaConfigDir -Force | Out-Null
    }
    ...
    $minConfig | ConvertTo-Json -Depth 4 | Set-Content -Path $NexaConfigFile -Encoding UTF8
}
...
$NexaCredDir = Join-Path (Join-Path $env:USERPROFILE ".nova-nexa") "credentials"
$NexaKeyFile = Join-Path (Join-Path $env:USERPROFILE ".nova-nexa") "secrets\credential_key"

# Check if NEXA_CREDENTIAL_KEY already exists (from env, file, or User env var)
$credKey = $env:NEXA_CREDENTIAL_KEY
$credKeySource = ""

if ($credKey) {
    $credKeySource = "environment"
} elseif (Test-Path $NexaKeyFile) {
    $credKey = (Get-Content $NexaKeyFile -Raw).Trim()
    $env:NEXA_CREDENTIAL_KEY = $credKey
    $credKeySource = "file"
}

# Backward compat: check User env var (legacy PS1 installs)
if (-not $credKey) {
    $credKey = [System.Environment]::GetEnvironmentVariable("NEXA_CREDENTIAL_KEY", "User")
    if ($credKey) {
        $env:NEXA_CREDENTIAL_KEY = $credKey
...
if ($credKey) {
    switch ($credKeySource) {
        "environment" { Write-Ok "NEXA_CREDENTIAL_KEY already set" }
        "file"        { Write-Ok "NEXA_CREDENTIAL_KEY loaded from $NexaKeyFile" }
        "user_env"    { Write-Ok "NEXA_CREDENTIAL_KEY loaded from User environment variable" }
    }
} else {
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0 -and $generatedKey) {
            ...
            $secretsDir = Split-Path $NexaKeyFile -Parent
            New-Item -ItemType Directory -Path $secretsDir -Force | Out-Null
            [System.IO.File]::WriteAllText($NexaKeyFile, $generatedKey)

            # Restrict file permissions (best-effort on Windows)
            try {
                $acl = Get-Acl $NexaKeyFile
                $acl.SetAccessRuleProtection($true, $false)
                $rule = New-Object System.Security.AccessControl.FileSystemAccessRule(
                    $env:USERNAME, "FullControl", "Allow")
                $acl.SetAccessRule($rule)
                Set-Acl $NexaKeyFile $acl
            } catch {
                # Non-critical; file is in user's home directory
            }

            $env:NEXA_CREDENTIAL_KEY = $generatedKey
            $credKey = $generatedKey
            Write-Ok "Encryption key saved to $NexaKeyFile"
        } else {
            Write-Warn "failed"
            Write-Warn "Credential store will not be available."
            Write-Host "  You can set NEXA_CREDENTIAL_KEY manually later."
        }
    } catch {
...
if ($credKey) {
    $credCredsDir = Join-Path $NexaCredDir "credentials"
    $credMetaDir  = Join-Path $NexaCredDir "metadata"
...
$credStoreDir = Join-Path (Join-Path (Join-Path $env:USERPROFILE ".nova-nexa") "credentials") "credentials"
...
if ($FrontendBuilt) {
    ...
    if ($SelectedProviderId -or $credKey) {
        ...
        if ($credKey) {
            Write-Host "- NEXA_CREDENTIAL_KEY is set for credential encryption"
        }

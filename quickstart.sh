...
echo ""
echo -e "${BOLD}        N O V A   N E X A${NC}"
echo ""
...
with open('$NEXA_CONFIG_FILE', 'w') as f:
...
PREV_ENV_VAR=""
PREV_SUB_MODE=""
if [ -f "$NEXA_CONFIG_FILE" ]; then
    eval "$($PYTHON_CMD -c "
import json, sys
try:
    with open('$NEXA_CONFIG_FILE') as f:
        c = json.load(f)
    llm = c.get('llm', {})
...
if [ -n "$NEXA_CREDENTIAL_KEY" ]; then
    echo -e "${GREEN}  ✓ NEXA_CREDENTIAL_KEY already set${NC}"
elif [ -f "$NEXA_KEY_FILE" ]; then
    NEXA_CREDENTIAL_KEY=$(cat "$NEXA_KEY_FILE")
    export NEXA_CREDENTIAL_KEY
    echo -e "${GREEN}  ✓ NEXA_CREDENTIAL_KEY loaded from $NEXA_KEY_FILE${NC}"
else
    ...
    if [ -z "$GENERATED_KEY" ]; then
        ...
        echo -e "${YELLOW}    You can set NEXA_CREDENTIAL_KEY manually later.${NC}"
    else
        echo -e "${GREEN}ok${NC}"

        # Save to dedicated secrets file (chmod 600)
        mkdir -p "$(dirname "$NEXA_KEY_FILE")"
        chmod 700 "$(dirname "$NEXA_KEY_FILE")"
        echo -n "$GENERATED_KEY" > "$NEXA_KEY_FILE"
        chmod 600 "$NEXA_KEY_FILE"
        export NEXA_CREDENTIAL_KEY="$GENERATED_KEY"

        echo -e "${GREEN}  ✓ Encryption key saved to $NEXA_KEY_FILE${NC}"
    fi
fi

# Create credential store directories
if [ -n "$NEXA_CREDENTIAL_KEY" ]; then
    mkdir -p "$NEXA_CRED_DIR/credentials"
    mkdir -p "$NEXA_CRED_DIR/metadata"

    # Initialize the metadata index
    if [ ! -f "$NEXA_CRED_DIR/metadata/index.json" ]; then
...
echo ""
echo -e "${GREEN}${BOLD}        NOVA NEXA — READY${NC}"
echo ""
echo -e "${GREEN}⬢${NC}${DIM}⬡${NC}${GREEN}⬢${NC}${DIM}⬡${NC}${GREEN}⬢${NC}${DIM}⬡${NC}${GREEN}⬢${NC}${DIM}⬡${NC}${GREEN}⬢${NC}${DIM}⬡${NC}${GREEN}⬢${NC}${DIM}⬡${NC}${GREEN}⬢${NC}${DIM}⬡${NC}${GREEN}⬢${NC}${DIM}⬡${NC}${GREEN}⬢${NC}${DIM}⬡${NC}${GREEN}⬢${NC}${DIM}⬡${NC}${GREEN}⬢${NC}${DIM}⬡${NC}${GREEN}⬢${NC}${DIM}⬡${NC}${GREEN}⬢${NC}"
echo ""
...
if [ "$FRONTEND_BUILT" = true ]; then
    ...
    echo ""
    echo -e "  ${DIM}Tip: You can restart the dashboard anytime with:${NC} ${CYAN}nexa open${NC}"
    echo ""
    # exec replaces the quickstart process with nexa open
